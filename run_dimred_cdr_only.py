"""
Same as run_dimred.py but restricted to CDR1/CDR2-derived features only
(drops the FR1+FR2+FR3 biochemical stats and composition vector), to isolate
whether CDR-specific properties alone.
still separate species, versus needing framework composition to do so.
"""
import numpy as np
import pandas as pd
import matplotlib
...
def run(locus='IGH', out_dir='/mnt/user-data/outputs', perplexity=30, random_state=42,
    ...
    cdr_cols = [c for c in feat_full.columns if c.startswith('CDR1_') or c.startswith('CDR2_') or c.startswith('CDR_comp_')]
    ...
    fig.suptitle(f'{locus}V gene CDR1/CDR2-ONLY feature space, colored by species (n={len(ann)} genes, {feat.shape[1]} features)', fontsize=13)
    axes[1].legend(bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=7, ncol=1, title='Species')
    fig.tight_layout()
    fig.savefig(f'{out_dir}/{locus}_Vgene_dimred_by_species_CDRonly.png', dpi=150, bbox_inches='tight')
    plt.close('all')
